...
Verify Azure CLI installation with `az version`; if missing, use the [official installation guide](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli). This extra prerequisite applies to user publishing only, not mock development, offline preflight, or explicit-tenant SPN updates. Neither installation nor Azure login is automatic.
