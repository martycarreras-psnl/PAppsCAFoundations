...
```bash
# Is pnpm itself available? Check this FIRST — the two checks below are
# meaningless without it, and `corepack` cannot always self-provision.
command -v pnpm >/dev/null && pnpm --version || echo "❌ pnpm missing — see below"
...
If `pnpm` is missing, that is the **only** thing to fix first — do not report the
other two, since they cannot pass without it. This repo pins `pnpm` via the
`packageManager` field in the root `package.json`, so install that exact version:

```bash
npm install -g pnpm@<version from packageManager field>
```

`corepack pnpm ...` is the documented alternative, but it downloads the package
manager on first use and fails on a restricted or offline network — prefer the
explicit `npm install -g` when corepack errors. Unlike Node.js or the .NET SDK,
this one **is** safe for you to run on the user's behalf: it is a userland npm
global install with no admin rights, no EULA, and no PATH manipulation.

If either of the other two checks fails, output:

```
🛑 Monorepo source tree — workspace not ready

You asked to run the wizard from source (`pnpm --filter ...` / `node packages/...`),
which needs pnpm present and the workspace installed and built first:

  npm install -g pnpm@<version from packageManager field>   # only if pnpm is missing
  pnpm install
  pnpm --filter @pacaf/wizard-ux build

After all succeed, re-run your command.

NOTE: if you just want to *use* the wizard, you don't need any of this —
`npx @pacaf/wizard-ux@latest` is self-contained, works from any cwd, and falls
...
**Never treat `pnpm` as a general prerequisite.** It is deliberately absent from the Step 1 precheck and from the wizard's own prerequisite step, because a consumer scaffolding a Code App genuinely does not need it — the scaffold prefers `pnpm` when it is on PATH and falls back to `npm` when it is not. Demanding `pnpm` from someone who is simply building an app is a false alarm.
