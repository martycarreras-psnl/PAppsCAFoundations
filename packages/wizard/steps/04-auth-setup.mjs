...
export default async function stepAuthSetup() {
  ...
  const authProfileType = stateGet('AUTH_PROFILE_TYPE', 'user');

  if (!devUrl) {
    ui.fail('Dev environment URL must be set (Step 2) before auth setup.');
    process.exit(1);
  }

  const pac = pacPath();
  if (!pac) {
    ui.fail('PAC CLI not found. Install it before creating auth profiles.');
    process.exit(1);
  }

  // ── User credentials flow ──
  if (authProfileType === 'user') {
    ui.line('Creating a user auth profile via interactive sign-in.');
    ui.line('');
    ...
    if (signInMethod === 'browser') {
      ...
      ui.line('If this terminal appears frozen for more than ~2 minutes, press Ctrl+C and pick device code instead.');
      ui.line('');
      userOk = runSafeLive(pac, ['auth', 'create', '--name', userProfileName, '--environment', devUrl], { timeout: 180000 });
      if (!userOk) {
        ui.warn('Browser sign-in did not complete. Falling back to device code.');
        ui.line('');
      }
    }
    ...
    if (!userOk) {
      ...
      ui.line('');
      userOk = runSafeLive(pac, ['auth', 'create', '--name', userProfileName, '--environment', devUrl, '--deviceCode']);
    }

    if (userOk) {
      ui.ok(`User profile ${userProfileName} created`);
    } else {
      ui.fail(`Could not create user profile. Try again later:`);
      ui.line(`  ${pac} auth create --name ${userProfileName} --environment ${devUrl} --deviceCode`);
      process.exit(1);
    }

    // Write .env with just environment URLs (no secrets)
    const dateStr = new Date().toISOString().slice(0, 10);
    let envContent = `# .env — Environment URLs for Power Platform.\n`;
    ...
    ui.ok('Wrote .env with environment URLs');
