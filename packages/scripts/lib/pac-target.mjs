...
const ENVIRONMENT_ID_PATTERN = '(?:default-)?[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}';
...
export function parsePacOrgWho(output = '') {
  ...
  const environmentIdMatch = text.match(new RegExp(`Environment\\s*Id\\s*[:=]\\s*(${ENVIRONMENT_ID_PATTERN})(?![a-z0-9-])`, 'i'))
    || text.match(new RegExp(`/e/(${ENVIRONMENT_ID_PATTERN})/`, 'i'));
...
export function extractPowerConfigTargetMetadata(config = {}) {
  ...
  const appUrlEnvironmentIdMatch = appUrl.match(new RegExp(`/play/e/(${ENVIRONMENT_ID_PATTERN})/app/`, 'i'));
...
export function solutionExistsInSelectedEnv({ pac, uniqueName, cwd } = {}) {
  ...
  const tmp = join(process.cwd(), `.pac-solcheck-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.xml`);
  let out;
  try {
