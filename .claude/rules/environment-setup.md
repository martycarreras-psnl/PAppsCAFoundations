...
CLI 1.0.2 status exposes `activeAccount.username` and `activeAccount.homeAccountId`, not the token's resource tenant. Even home-tenant equality is not tenant evidence. Connected scaffold initialization and user publishing therefore require **separate, explicit Azure CLI login** with discovery access to the target (`az login --allow-no-subscriptions --tenant "<expected-tenant-id>"`). The helper makes a read-only `az rest` call to its fixed cloud-specific Global Discovery Service endpoint and matches `EnvironmentId`, `TenantId`, and `Url` to the target.

Azure CLI is a prerequisite **for connected scaffold initialization and user publishing**, not for mock development, offline preflight, or the explicit-tenant SPN update path. Verify installation with `az version`; if unavailable, direct the user to the [official Azure CLI installation guide](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli). Do not install system tools or initiate Azure login automatically.
