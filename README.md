...
2. **`packages/wizard-ux/dist/` is generated and shipped, not committed.** Git ignores the prebuilt React UI bundle; the package's `files` list includes it in npm tarballs. Regenerate it via `pnpm -r run build` before packing or publishing (the Release workflow does this).
