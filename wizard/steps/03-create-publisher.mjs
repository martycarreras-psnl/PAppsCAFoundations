// wizard/steps/03-create-publisher.mjs — Power Apps Maker Portal guidance for publisher creation
...
export default async function stepCreatePublisher() {
  ...
  ui.line('This step happens in the Power Apps Maker Portal. The wizard can\'t do it');
  ...
  const choicePrefix = await input({
    message: 'Choice value prefix (4–6 digit number shown in the Power Apps Maker Portal)',
    validate: (v) => {
  ...
  if (!done) {
    ui.line('');
    ui.line('No problem — complete it in the Power Apps Maker Portal and re-run the wizard.');
